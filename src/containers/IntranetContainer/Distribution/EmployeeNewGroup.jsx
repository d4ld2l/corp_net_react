...
const cn = require('bem-cn')('new-group-employee')
...
class EmployeeNewGroup extends PureComponent<Props, State> {
  state = {
    fetching: false,
    results: [],
    dropdown: false,
  ...
  Item = ({
    ...
    return (
      ...
      >
        ...
        <div className={cn('item-name').mix('p1 p1_theme_light_first')}>
          {it.name()}
          {it.isUser() && [
            it.position() && (
              <div key="position" className={cn('item-position').mix('p2 p2_theme_light_second')}>
                {it.position()}
              </div>
            ),
            // it.departments() && (
            //   <div key="departments" className={cn('item-departments').mix('p2 p2_theme_light_second')}>
            //     {it.departments()}
            //   </div>
            // ),
          ]}
    ...
    return (
      <div className={cn}>
        <div className={cn('group').mix('form-group')}>
          <h3 className={cn('group-name').mix('indent_5')}>Участники</h3>
          <label className={cn('group-label', { disabled: checked }).mix('p3 p3_theme_light_second')} htmlFor={'name'}>
            Выбрать сотрудника
          </label>
        ...
        <div className={cn('check-all')}>
          ...
          >
            <span className={cn('check-all-label').mix('p1 p1_theme_light_first')}>Все сотрудники</span>
          </Checkbox>
        </div>
        {!checked && (
...
export default connector(EmployeeNewGroup)
