        ...
        <div key={index} className={cn('item').state({ hidden: fields.get(index)._destroy })}>
          ...
          <div className={cn('info')}>
            <div className={cn('name').mix('p1 p1_theme_light_first indent-reset')}>
              {fields.get(index).full_name}
            </div>
            <div className={cn('position').mix('p2 p2_theme_light_second')}>
              {fields.get(index).position}
            </div>
            <div className={cn('unit').mix('p2 p2_theme_light_second')}>
              {fields.get(index).department}
            </div>
          </div>
          <Field
