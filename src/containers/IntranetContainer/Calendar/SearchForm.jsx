...
const cn = require('bem-cn')('search-form-event')

if (process.env.BROWSER) {
  require('./search-form-event.css')
}
    ...
    return (
      ...
      >
        ...
        <div className={cn('dropdown-item-name')}>
          ...
          {it.isUser() &&
            it.position() && <div className={cn('dropdown-item-position').mix('p2 p2_theme_light_second')}>{it.position()}</div>}
        </div>
      </div>
    )
  }
            ...
            {this.state.dropdown &&
              ...
              this.state.results.length === 0 && (
                <div className={cn('input-dropdown')}>
                  <div className={cn('dropdown-item-placeholder').mix('p2 p2_theme_light_second')}>Не найдено</div>
                </div>
              )}
          </div>
