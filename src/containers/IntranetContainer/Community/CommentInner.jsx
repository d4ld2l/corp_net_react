...
import { deleteComment, dislikeComment, likeComment } from '../../../redux/actions/feedsActions'
...
class CommentInner extends Component {
  render() {
    ...
    return (
      <section className={cn('comment-wrapper').mix('publication__comment-wrapper_inner')}>
        <div className={cn('data')}>
          {child.account ? (
            <img
              className={cn('avatar')}
              src={child.account.photo.thumb.url}
        ...
        <div className={cn('control-panel-comment')}>
          <div className={cn('wrapper-icon')} title="Мне нравится">
            ...
            <span className={cn('count').mix('p4 p4_theme_light_second')}>{child.likes_count}</span>
          </div>
        </div>
      </section>
    )
