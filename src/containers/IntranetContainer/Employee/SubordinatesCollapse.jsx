...
export default class SubordinatesCollapse extends Component {
  state = {
    open: false,
  }
  render() {
    ...
    let employees = []
    if (current.subordinates_list) {
      employees = [].concat(current.subordinates_list)
    }
    employees.reverse()
    ...
    return (
      <div className={cn} id="statuses">
        <div className={cn('head')} onClick={() => this.setState({ open: !open })}>
          <h3 className={cn('title').mix('fw_400')}>Подчиненные</h3>
                ...
                <Col xs={12}>
                  {employees.map(element => (
                    <div className={cn('card')} key={Math.random()}>
                      <figure className={cn('image-personnel')}>
                        <img
                          src={
