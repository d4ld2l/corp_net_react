        ...
        <div className={cn} id="education">
          <div className={cn('head')} onClick={() => this.setState({ open: !open })}>
            <h2 className="indent-reset">
              ...
              {current.resumes[0] && current.resumes[0].resume_certificates.length}
            </sup></h2>
          ...
          <Collapse in={this.state.open}>
            <div>
              <div className={cn('body')}>
                {(current.resumes[0] && current.resumes[0].resume_certificates.length > 0) && (
                  <h4 className={cn('margin-head-4')}>Сертификаты</h4>
                )}
                {current.resumes[0] && current.resumes[0].resume_certificates.map(item => (
                  <article className={cn('achievement-article')} key={item.id}>
                    <div className={cn('date-interval')}>
                      <p className={cn('achievement-date').mix('p1 p1_theme_light_second')}>{item.start_date ? moment(item.start_date).format('YYYY') : ''}</p>
                    </div>
