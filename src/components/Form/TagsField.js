...
export default class TagsField extends Component {
  ...
  addValue = () => {
    if ((this.state.values && this.state.values.map(item => (item.name)).indexOf(this.state.value.trim()) < 0) && (this.state.value.trim() !== '')) {
      this.setState({values: this.state.values.concat({name: this.state.value.trim()})}, this.setValues)
    } else {
      this.setValues()
    }
  }
          ...
          {this.state.values && this.state.values.map(item => {
            if (!item._destroy){
              return (
                ...
                {!isEmpty(item.name) ? item.name : item}
                  {
